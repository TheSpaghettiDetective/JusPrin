...
void load_and_print_preset(const std::string& file_path) {
    try {
        ...
        if (dir_name == "machine" || dir_name == "printer") {
            ...
            Slic3r::ConfigSubstitutions substitutions = preset_bundle.load_config_file(file_path, Slic3r::ForwardCompatibilitySubstitutionRule::EnableSilent);
            if (!substitutions.empty() || preset_bundle.prints.size() > 0 || preset_bundle.filaments.size() > 0 || preset_bundle.printers.size() > 0) {
