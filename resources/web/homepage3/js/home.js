...
function OnConnectToServer() {
    // Check if we need to redirect to the preloaded page
    if (typeof SendMsg_InitServerURLAndRedirect === 'function') {
        // If the function exists in the native environment, use it
        SendMsg_InitServerURLAndRedirect();
    } else {
        // Otherwise, try to load preloaded.html directly
        const resourcesDir = getResourcesDir();
        const preloadUrl = `file://${resourcesDir}/web/homepage3/preloaded.html`;
        window.location.href = preloadUrl;
    }
}

function SendMsg_InitServerURLAndRedirect() {
    // This function should be implemented in the native application
    // It will be called to initialize the server URL and redirect to the preloaded page
    let msg = {
        'command': 'init_server_url_and_redirect',
        'target': 'homepage'
    };
    SendToHost(msg);
}
...
function getResourcesDir() {
    // Determine the resources directory dynamically based on the current URL
    // Assuming the current URL is something like file:///path/to/resources/web/homepage3/index.html
    const currentUrl = window.location.href;
    const pathParts = currentUrl.split('/');

    // Remove the last two parts (homepage3/index.html) and join the rest
    pathParts.pop(); // Remove index.html
    pathParts.pop(); // Remove homepage3 directory

    return pathParts.join('/');
}

// Handle messages from the server connection component
window.addEventListener('message', function(event) {
    if (event.data && event.data.action === 'set_server_url') {
        window.SERVER_URL = event.data.url;
