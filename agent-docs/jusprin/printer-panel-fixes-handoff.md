...
**Implemented 2026-09-18, "‹ Printers" and "Set it up myself" only.** "Has content" is `state.messages.length > 1` in `AgentUI/src/App.tsx` (the panel always seeds exactly one opening line before anything else can happen, so one message alone is untouched); a shared `printerActionGated` wrapper shows the dialog for `close`, and for `manual_setup` in Add mode only (a Change session's own "Set it up myself" stays in the session, never gated). The dialog reuses `ChatNavigation.tsx`'s existing `.chat-dialog-shade`/`.chat-dialog`/`.chat-dialog-buttons` pattern verbatim -- same classes, same Escape handling -- no new dialog style or CSS. "After a printer is added, closing needs no confirmation" needed no change: `add_proposed_printer` already calls `close_panel()` directly in C++, never through the page's `close` action this gate sits in front of.

**Not implemented: "opening another printer's conversation from Home while one is open."** That path is native-initiated -- `ShellController` calls `PrinterPanel::open()` directly in response to a Home click, and `open()` unconditionally tears down whatever session was running (`PrinterPanel.cpp`). Confirming it would mean pausing that native call until an async answer comes back from the page, a materially different shape than the page-driven `close`/`manual_setup` cases above, which this session didn't build. Whoever picks this up: the check is the same (`AgentHost`'s conversation has more than the opening message), but the plumbing has to go the other direction, native asking the page rather than the page deciding on its own.
