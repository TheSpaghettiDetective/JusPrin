...
- **The printer panel's session is the exception to "every tool loads".** Home's printer panel runs its own conversation with exactly one tool per mode: `printer_identify` when adding a printer, `printer_change` when changing one (`PrinterConversation::session_tools`). A Change session that could also identify a printer could clear the card of the printer it is about. Measured 2026-09-18 on the app's own request bodies against `gpt-5.4-mini`: Add sends one definition of 597 bytes and instructions of 28,425 bytes (the rules plus the 383-line printer list), about 11,540 input tokens per request, of which 11,008 came back cached on every request after the first of the day; the cold first request cached nothing. Change sends one definition of 832 bytes and about 552 input tokens, below the size OpenAI caches at all. Neither session carries the project: no workspace in the turn, none in a tool result.
...
- the printer panel's two tools carry only the `Printer` exposure: neither adapter lists them, and the panel's session offers one of them per mode. They take no `planId` (`joins_plans` excludes them), and what the model reads back from them is the tool's own result, not the host's workspace envelope.
...
|---|---|---|---|---|---|---|
| `activity_cancel` | Stop a slice, a job or a waiting proposal the tools started | mutation (computation-only) | both | one handle from `slice_start`, a job, or a proposal's action id | `Plater::cancel_slicing`, the job worker's cancel, the coordinator's own cancel | the handle, its kind, whether the cancel won the race, and a message |
| `export_file` | Write the G-code or another file where the user wants it | destructive | both | a sliced plate, the project, placed objects or the presets in use; a path only as an approved input | the plate's G-code output, `Plater::export_3mf` with `Silence`, raw part meshes, `PresetBundle::export_current_configs` | 32 files, bytes, license and whether it restricts use, and for G-code and sliced 3MF up to 8 of the slice's own warnings (also on the card) |
...
| `object_divide` | Cut or split an object for printing | mutation | both | one object; one undo step | `Plater::apply_cut_object_to_model` after Orca's `Cut`, `Plater::split_object`, `ModelVolume::split` | 64 pieces as they now lie, overhang area before and after, 32 `regionsUnbound` |
| `object_divide_preview` | See the pieces before cutting | read-only | both | one object in a scratch model | `Cut`, `ModelObject::split` / `ModelVolume::split` on a copy | 64 pieces as they would lie, overhang area before and after (an estimate, not a slice), 32 regions that would unbind |
...
| `object_merge` | Join objects into one | mutation | both | 2 to 32 objects; one undo step | the object list's assemble | the merged object id and 32 `regionsUnbound` |
...
| `object_repair` | Close an object's open mesh | mutation | both | one object; one undo step | the repair command's per-part CGAL step, without its progress dialog | whether it changed; open edges, facets, parts and volume before and after; 32 `regionsUnbound` |
...
| `printer_change` | Change a printer the person owns: the nozzle size now on it, or the spools loaded | mutation | printer panel, Change mode only | the one saved printer the panel is about; never the open project | `Printers::change_named_printer_nozzle` (writes the profile without selecting it), `SpoolStore` | `state` applied or declined, `changed` with before and after, the printer as it now is; checked before its card (`unknown_nozzle`, `nothing_to_change`, `printer_gone`), and Undo reverses it without a card |
| `printer_identify` | Show the printers the model decided the person means | read-only | printer panel, Add mode only | 1 to 3 ids copied from the printer list in the instructions | `PrinterCatalog::panel_printers` | each printer's brand, model, build volume, nozzle sizes and what its card assumes (nozzle; plate and filament, `null` where the profile names none); `too_many`, `unknown_printer`, `unknown_nozzle` |
...
| `project_attachment_read` | Read a picture or note packed in the project | read-only | both | one relative path inside the attachment folder, outside `JusPrin/` and `.thumbnails/` | the project's auxiliary folder; `wxImage` to scale | one image (1280 px, 2 MB) or 32 KB of text |
...
| `project_save` | Keep the work, under its own name or a new one | destructive | both | the open project; a path only as an approved input | `Plater::save_project` after `set_project_filename`, JusPrin state flushed first | the saved path, the dirty flag and the revision; a path not ending in `.3mf` is refused before the card and pointed at `export_file` |
...
| `view_render` | See the plate | read-only | both | one plate, prepare mode | `GLCanvas3D::render_thumbnail` with the 3MF thumbnail's parameters | one PNG, 64 to 1280 px a side, beside its size and mime type |
| `workspace_inspect` | Client needs current project context | read-only | both | current document | `IWorkspace::snapshot`, plus `IProductState` for intent and plan | at most 16 plates, 64 objects across returned plates and 64 selected IDs; labels at most 256 UTF-8 bytes; totals and truncation flags; `activities` lists the latest 20 tool calls with state, plan and error |
...
**Seeing and exporting (M6).** A tool activity may carry a `ToolImage` (base64 PNG or JPEG, capped at 1280 px and 2 MB) that is never persisted; the MCP projection appends an `image` content block (the bridge passes it through on every protocol version), and the OpenAI projection follows the function output with a user message holding an `input_image`, both tested. `view_render` calls `GLCanvas3D::render_thumbnail` with the main 3MF thumbnail's parameters (the plate's printable parts, no bed) and Orca's `Camera::ViewAngleType`, and encodes with `GCodeThumbnails::compress_thumbnail`; it refuses before the canvas is initialized, as the archive export does. Preview mode is not built. `project_attachment_read` reads only a relative path inside the attachment folder, outside `JusPrin/` and `.thumbnails/`; larger pictures are scaled and re-encoded with `wxImage`, text is cut at 32 KB. `slice_inspect` rebuilds the preview's layer split from `GCodeProcessorResult::moves` (the stored result has no layer list or layer times) and pages the plate's G-code file by line. `activity_cancel` is computation-only: a pending proposal through the coordinator's own cancel, the tools' slice through `Plater::cancel_slicing` (the notification's Cancel, which returns once the run stopped; the snapshot's running flag clears a moment later), and a tools' job through the worker's cancel, whose end is read from the jobs list. `export_file` writes G-code by copying the plate's slicer output (Orca's own export adds only post-processing scripts and removable-media handling, which this does not run), sliced and project 3MF through `Plater::export_3mf` with `Silence` (so the open project keeps its name and path), STL from the parts' raw meshes as placed (negative volumes are not subtracted), and presets through `PresetBundle::export_current_configs` with system presets included, because the presets in use are usually system presets with edits and Orca's menu would write nothing for them. `slice_start` takes `wait`, and `slice_report` an `intent` section that measures the slice against intent answers naming a time, a weight or a cost. The shell harness renders a front view, reads a packed picture scaled from 2000 px and a text file, refuses `..` and JusPrin's state, reads layers and G-code, writes all five export kinds without a dialog, refuses an unasked replace, a relative path and the data folder, and stops a running slice.

**M6 eval, 2026-09-17, Windows, fake Bambu A1 mini.** Over MCP: the T bar sliced with `slice_start` `wait` (finished), its report read (2,011 s, 9.37 g), an iso picture rendered (10 KB PNG) and 180 layers inspected; a G-code export was rejected and nothing was written, then approved and written (897 KB, starting with a comment); a sliced 3MF was exported and reopened with `project_open`, and its report gave the same time and weight — pass. In-app with OpenAI, one run ("show me a front view and tell me what you see, then slice it and export the G-code to ..."): the agent rendered the front view and described it correctly (a T-shaped part with a round hole near the top, centred and upright), which exercises the image-input projection against the real API, sliced with `wait`, and exported after approval — pass. No dialog appeared in either. After M6: in-app 30 definitions and 33,153 bytes, MCP 31 and 37,528 over two pages.

**Grouped approvals (M7).** The registry adds an optional `planId` (1 to 64 characters) to every mutation's schema except `plan_set` (`joins_plans`), so those tools use non-strict function calling; `validate_call` checks it, refuses it on a read, keeps it away from the tool's own decoder and puts it back for the coordinator, which records it on the activity (saved in `state.json`, sent to the page as `planId`). A member always waits for approval, including a computation-only call that would otherwise run without a card. `plan_set` is left out because a live run read "make these changes as one plan" as a call to it with a `planId`, and grouping the agent's own words adds nothing to the card. Both adapters answer a member at once instead of waiting for the card, because the person can decide only after the agent has proposed the rest: the in-app agent gets `{"state":"queued", "planId", "actionId", "message"}` as its function output, and an MCP client gets the same text with no `structuredContent` (it is not the tool's result; `_meta["io.jusprin/activity"].state` is `queued`). The server advertises no `outputSchema`, so no client expects structured content from it. The outcome is read later from the new `activities` section of `workspace_inspect` (the latest 20 calls with state, plan and error). `approve` or `reject` on any member decides every waiting member; `pump` runs a plan's members in proposal order, one at a time, and holds a later member while an earlier one is still running (a slice that waits). Staleness for a member counts only changes from outside the plan: a change that arrives while another member of the same plan executes does not count, and any other content or settings change after approval fails the next member as `stale_revision`. Settings patches in a plan skip the revision half of their own guard for the same reason; the confirmed before-values still guard every key. A member that fails fails the plan's remaining approved members as `plan_step_failed`; members already done stay done. The page draws one card per plan beneath the message that proposed its first member (and in the external-tools area for MCP plans), titled with the latest `plan_set` headline, listing each member with its state, naming the member that makes the plan destructive, with one Approve all / Reject all that sends the decision for the first waiting member. While the in-app agent's turn is still streaming, those buttons are disabled ("The Agent is still adding to this plan"), so a person cannot approve half a plan. An MCP plan has no such signal; its card is live as soon as its first member arrives.

A plan takes members only while its card is undecided and none of its calls has failed; a later call with that `planId` is refused as `plan_closed` (a live run proposed a member after the plan's first call had already failed stale). The coordinator's own error codes for plans are `plan_closed` and `plan_step_failed`.

**Fixes after the review (2026-09-17).** A plan is its source, its scope and its id together (`same_plan`): the in-app agent's scope is the chat (`planScope`, saved and sent to the page), an MCP plan has none. Before this, an MCP client and the in-app agent choosing the same id shared one plan, and Approve all on the external card also ran in-app members it did not list. A plan with a single member is drawn as an ordinary card once the in-app agent has finished proposing (and always, for MCP): `gpt-5.4-mini` sometimes gave a lone settings patch a `planId`, the page then showed a plan card, and the live regression's single-card decision never landed (3 runs in 4). The `planId` description now says to leave it out for one change. A slice starting, ending, or stopping being current publishes `WorkspaceChangeReasons::Slicing`, which is not in `kInvalidatingReasons`: it used to publish `Plates`, so the agent's own card-free `slice_start` failed every waiting card and a plan could not continue after its own slice. `Slicing` fails only a waiting `export_file` of a sliced kind, whose card names that slice's warnings. Still open: a plan cannot slice and then export, because `export_file` checks for a current slice when it is proposed, before the plan's slice has run.

**What the journey eval changed (M7).** `slice_report` findings follow Orca's send dialog: the bed-temperature advisory (which Orca never shows) and the older A-series copy of the traditional-timelapse warning are dropped, and the timelapse warnings carry `appliesWhen: "timelapse"` and are not critical, since a G-code export records no timelapse. An `export_file` card for G-code or a sliced 3MF names the slice's own warnings, and the result lists them as `sliceWarnings`. `project_save` refuses a path that is not a `.3mf` before the card and points at `export_file` (a live run tried to save the project over the G-code path). A refused call now names values outside a listed set and says when `project_open` got both `path` and `new`. The page's state push no longer shows the `project_open` that replaced the project inside the project it opened (the call is still live while it finishes; its record belongs to the closed session). Both adapters' instructions carry one shared sentence on how a print request goes (`kPrintJourneyGuidance`): record the intent, ask only consequential questions, record the plan and its assumptions with `plan_set` before changing the project, check the slice, export with `export_file`. The live regression (`--live-agent`) gained a plan stage: the real model queues two settings patches under one `planId`, the harness clicks the plan card's Approve all, and both apply in order with the second not stale.

**Final journey eval, 2026-09-17, Windows, fake Bambu A1 mini, twice.** The model is Orca's bundled Cali Cat, written out as an STL by `export_file` (28.5 × 28.5 × 35 mm, one object).

Over MCP (the scripted client in place of Claude Code, per the owner's instruction to use OpenAI only; every approval clicked in the JusPrin panel, the plan's on its own card button): open the STL as a project; `intent_update` with decorative, front face visible, under five hours, PLA; `object_analyze`; `plan_set` with the orientation and seam decisions and two assumptions; one plan card holding `region_annotate` (faces toward −Y visible, faces toward +Y hidden) and `settings_apply_patch` (`seam_position` back), both answered queued, one card with the headline and one Approve; both ran; `slice_start` with `wait`; `slice_report`: 1,824 s against the 18,000 s intent limit (the intent check measured it), no finding that applies to a G-code print (the timelapse advisory is reported with `appliesWhen`), seams 0 on the visible face and 274 of 296 on the hidden one; `settings_search` `changedOnly` lists only `seam_position`, the plan's own change; the G-code written after approval (950 KB) — pass, twice (before and after the last fixes). No dialog appeared.

In-app with OpenAI (`gpt-5.4-mini`), seven runs of "It's decorative, the front face will be visible, it needs to print in under five hours, in PLA. Get me to an exported G-code file at ...", with the STL:

- Runs 1 and 2 named its path, so the agent opened it with `project_open`. That ends the chat it was asked from (the chat belongs to the closed project), and the page kept a stale running card; fixed as above. The first attempt was also refused twice for giving `path` with `new`, and the refusal did not say why; it does now. From run 3 on the STL is attached to the chat of an empty project, as in M3.
- Run 3 put `project_save` with the `.gcode` path in a plan, skipped `plan_set`, and said the export was queued; the save failed after approval. Fixed as above.
- Runs 4, 5 and 7 recorded the intent and a plan with assumptions (run 4 twice), imported through a plan card, then turned the cat onto its side, which Orca's orientation cost ranks first (unprintability 0.103 against 0.216 upright), sliced and exported without reading the report. Orca then warns of a floating cantilever, so the exported slice has a warning: an eval failure. In run 7 the export card named that warning and the agent reported it.
- Run 6 met the whole request: intent recorded, `plan_set` with three assumptions, import and slice approved on one plan card, the cat left upright, the export approved, and the slice (1,819 s) has no finding that applies to a G-code print; no setting was changed by hand or by the agent. It asked no questions, which is right for this model: nothing in its geometry makes a choice consequential. Pass.

No in-app run asked a question; every run reached an exported file in two or three turns ("Approved. Carry on." after each plan card, because the agent ends its turn to ask for the approval). One run in seven met the whole request, and the recurring failure is an orientation choice the tools report truthfully but the model does not check. The catalog after M7: in-app 30 definitions and 36,195 bytes, MCP 31 and 40,714 over two pages; each mutation's `planId` costs about 100 bytes. The `--live-agent` regression: 25 requests, 7,249 to 11,619 input tokens, 72 to 95 per cent cached after the first request (the first was uncached because the catalog had just changed).
...
- The coordinator needs [grouped approvals](#grouped-approvals) and the `planId` field on every mutation (done in M7).
...
### Security review for the journey catalog (M6, 2026-09-17)

The journey catalog trips two of the triggers above. This is the written answer the gate asks for; the loopback-only design stands, and nothing new is built.

**Tools that read or write a caller-supplied path.** `project_open` and `object_import_file` read one; `project_save` and `export_file` write one. A local process that can reach the socket can propose any of them, and none of them touches the path until the person approves a card that shows the exact path (and says "replacing it" when a file is there). Approval is per call; nothing is remembered. The coordinator tests prove the file is untouched on rejection for each: `project_save` (the target is absent after a rejection), `project_open` (no open happens), `object_import_file` (no import happens), and `export_file` (an existing file keeps its contents after a rejection and is replaced only after an approval that named it). `export_file` is additionally destructive by class, so it can never use the computation-only exemption; it takes only an absolute path in an existing folder with the kind's extension, replaces a file only with `overwrite`, refuses OrcaSlicer's data folder and the project's temporary folder, and names a restrictive project license on the card and in the result. The shell harness writes all five kinds without a dialog and proves the refusals of an unasked replace, a relative path and the data folder. `project_attachment_read` takes no path: an id from the project's own attachment listing, resolved inside the attachment folder, with `..` and JusPrin's own state refused (harness).

**The computation-only exemption.** `slice_start`, `plan_set` and `activity_cancel` run without a card. `slice_start` replaces a computed result and refuses to take over a run it did not start unless `preempt`, which brings the card back. `plan_set` records the agent's own statement, shown in the plan card, and changes nothing in the project. `activity_cancel` stops only what the tools started: a waiting proposal, a job the tools queued, or the slice `slice_start` started while that run lasts; once that run has ended, a slice the person starts is not stopped by the old handle (coordinator test). The person can start any of these again.

**Not tripped here.** No camera frames (`printer_camera_snapshot` is not built), no print upload (`print_send` is not built), no binding beyond loopback, no grants that outlive a restart.
