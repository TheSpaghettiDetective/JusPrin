...
`workspace_inspect` takes an optional `sections` list — `summary` (the default, and exactly what it returned before sections existed), `intent`, `plan`, `slicing`, `history`. The `history` section extends the summary's `history` object with `restorable` and the steps, newest 32. Its output schema requires only the identity fields, so a caller that asks for one section is not handed the rest. It is no longer a strict OpenAI function: strict mode cannot express an optional argument.
...
Only `workspace_inspect`, `settings_search`, `settings_get`, `settings_preview_patch`, `settings_apply_patch`, `intent_update`, `plan_set`, `slice_start`, `slice_report`, `presets_list`, `printer_list`, `project_save`, and `history_restore` are MCP-visible. `duplicate_object` and `inspect_selection` remain in-app fixtures pending an in-app eval; they are not callable over MCP. There is no MCP attachment-import contract.
