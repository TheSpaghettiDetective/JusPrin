...
Only `workspace_inspect`, `settings_search`, `settings_get`, `settings_preview_patch`, `settings_apply_patch`, `intent_update`, `plan_set`, `slice_start`, `slice_report`, `presets_list`, `printer_list`, `printer_setup_preview`, `printer_setup`, `project_open`, `project_save`, `history_restore`, `object_analyze`, and `object_place` are MCP-visible. `duplicate_object` and `inspect_selection` remain in-app fixtures pending an in-app eval; they are not callable over MCP. There is no MCP attachment-import contract.
