...
- **The baseline, measured on Windows against the live app.** After M2: in-app 18 definitions and 12,721 bytes, MCP 16 and 13,911 (13,894 through the offline helper recipe). After M1: in-app 11 definitions and 6,482 bytes, MCP 9 and 6,915. At M0 it was in-app 7 and 3,145 bytes, MCP 5 and 3,147.
...
Only `workspace_inspect`, `settings_search`, `settings_get`, `settings_preview_patch`, `settings_apply_patch`, `intent_update`, `plan_set`, `slice_start`, `slice_report`, `presets_list`, `printer_list`, `printer_setup_preview`, `printer_setup`, `project_open`, `project_save`, and `history_restore` are MCP-visible. `duplicate_object` and `inspect_selection` remain in-app fixtures pending an in-app eval; they are not callable over MCP. There is no MCP attachment-import contract.
