...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    for (size_t i = 0; i < data.size();) {
        if (i == 0 || data[i] == '\n') {
            ...
            for (; i < data.size() && (data[i] == ' ' || data[i] == '\t'); ++ i) ;
            if (i < data.size() && strncmp(data.data() + i, "generated by ", 13) == 0) {
                // Skip the "; generated by ..." line.
                for (; i < data.size() && data[i] != '\n'; ++ i);
            }
            // Handle comments starting with # (at the beginning of a line)
            else if (i < data.size() && data[i] == '#') {
                // Skip the entire comment line
                for (; i < data.size() && data[i] != '\n'; ++ i);
            }
            // If we're still at the start of a line but haven't processed anything, make sure to increment i
            else if (i < data.size() && (i == 0 || data[i-1] == '\n')) {
                // Copy the character and move forward
                data[j++] = data[i++];
            }
