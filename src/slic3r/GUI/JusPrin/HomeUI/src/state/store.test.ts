...
describe('the Home reducer', () => {
  ...
  it('carries the highlight only on the state that names it', () => {
    let s = reduce(initialState, {
      kind: 'envelope',
      envelope: envelope('state', { appearance: 'light', projects: [], printers: [printer], highlightPrinter: 'Bambu Lab A1 mini' }),
    });
    expect(s.highlightPrinter).toBe('Bambu Lab A1 mini');
    s = reduce(s, { kind: 'envelope', envelope: envelope('state', { appearance: 'light', projects: [], printers: [printer] }) });
    expect(s.highlightPrinter).toBeUndefined();
  });
});
