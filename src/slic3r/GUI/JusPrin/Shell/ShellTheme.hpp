...
// The text roles the token file defines under typography.roles, plus
// Technical: the system teletype face at the body size, reserved for
// measurements, temperatures, filenames and machine status.
enum class TextRole { PageTitle, Section, Body, Label, Metadata, Technical };
...
struct MenuRowMetrics   { int height{0}; int radius{0}; };
...
private:
    ...
    std::array<TypeStyle, 6> m_type_styles{};
    mutable std::array<wxFont, 6> m_fonts{};
};
