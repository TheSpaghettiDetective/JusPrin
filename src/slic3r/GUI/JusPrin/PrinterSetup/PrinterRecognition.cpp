...
bool DeterministicPrinterRecognition::start(std::uint64_t generation, const PrinterEvidence& evidence,
                                            const std::vector<const PrinterCandidate*>& choices)
{
    RecognitionEvent event;
    event.generation = generation;
    ...
    if (match_at == std::string::npos && description.find("touchscreen") != std::string::npos) {
        static const std::array<std::string, 2> figma_models = {
            "creality ender-3 v2", "creality ender-3 v2 neo"
        };
        for (const std::string& wanted : figma_models) {
...
std::optional<RecognitionEvent> DeterministicPrinterRecognition::poll()
{
